...
<script type="text/javascript">
	$(function(){
		...
		$('.addr-data').each(function(index) {
			var data = $(this).text()
			data = '<span style="width:160px;display:block;float:left;">' + '<span class="label label-info" data-toggle="tooltip" title="' + data + '">IP</span>' + data + '</span>';
			$(this).html(data);
		});
	...
	$(function() {
		...
		$('#container').highcharts('StockChart', {
				...
				events : {
					load : function() {
						...
						setInterval(function() {
							$.get('/logger/getlatestdata?zoneid='+$('#currentzone').val()+'&loggerid='+ $('#currentlogger').val(), function(data) {
			...
			title : {
				text : 'Performance Evaluation of logger: ' + $('#currentlogger').val()
			},
